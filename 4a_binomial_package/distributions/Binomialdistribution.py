...
class Binomial(Distribution):
    ...
    def replace_stats_with_data(self):
        ...
        number_of_positive_trials = [t for t in self.data if t==1]
        self.p = 1.0*(len(number_of_positive_trials))/self.n
    ...
    def plot_bar(self):
        ...
        x = ['tails', 'heads']
        counts = [(1-self.p)*n, (self.p)*n]
        x_pos = [0, 1]
        plt.bar(x_pos, counts, color=green)
        plt.xlabel('Heads and Tails')
        plt.ylabel('Count of Heads and Tails')
        plt.title('Histogram of instance varialbes')
        plt.xticks(x_pos, x) 
        plt.show()
    ...
    def pdf(self, k):
        ...
        #bin_coef = math.comb(self.n, k)
        bin_coef = math.factorial(self.n)/(math.factorial(k)*math.factorial(self.n-k))
        pdf = bin_coef * pow(self.p, k) * pow((1-self.p), (self.n-k))
        return pdf
    ...
    def plot_bar_pdf(self):
        ...
        pdf_for_x = [pdf(self, k) for k in range(self.n + 1)]
        x = [str(i) for i in range(n+1)]
        x_pos = [i for i in range(n+1)]
       
        plt.bar(x_pos, pdf_for_x)
        plt.xlabel("Values of x")
        plt.ylabel("PDF for x")
        plt.title("Binomail Probability Density Function")
        plt.xticks(x_pos, x)
        plt.show()
       
        return(x_pos, pdf_for_x)
    ...
    def __add__(self, other):
        ...
        new_pdf = Binomial(self.p, self.n + other.n)
        new_pdf.data = self.data + other.data
        return(new_pdf)             
